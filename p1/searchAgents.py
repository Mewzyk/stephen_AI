    ...
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      ...
      if not self.walls[nextx][nexty]:
        if (nextx, nexty) in state[1]:
          newTuple = tuple(x for x in state[1] if x != (nextx, nexty))
...
def cornersHeuristic(state, problem):
  ...
  path_distance, my_position, cornerList, cornerTotals = 0, state[0], list(state[1]), []
  temp_corners = []

  while cornerList:
    for index in range(0, len(cornerList)):
      cornerTotals.append(util.manhattanDistance(my_position, cornerList[index]))
      min_corner = cornerList[index]

    min_distance = min(cornerTotals)
    path_distance += min_distance
    cornerList.remove(min_corner)

  solution = path_distance - cornerDifference(state[1])
  return solution

def cornerDifference(input_corners):
  curr_max, counter = 0, 1
  for corner in input_corners:
    for other_corner in range(1, len(input_corners)):
      max(curr_max, abs( corner[0] - input_corners[other_corner][0] ) + abs( corner[1] - input_corners[other_corner][1] ))
  return curr_max
